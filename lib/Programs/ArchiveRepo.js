module.exports = (function() {
    ...
    ArchiveRepo.prototype.run = function() {
        ...
        .then(function (_repos) {
            repos = _repos;
            return commander.askForRepo (repos, 'Pick a repo to archive', 'name');
        })
        .then(function (_repo) {
            repoToArchive = _repo;
        ...
        .then(function (_branches) {
            branches = _branches;
            return commander.askForBranch (branches, 'Pick a branch from ' + organization + '/' + repoToArchive + ' to archive', 'branch');
        })
        .then(function (_branch) {
            branchToArchive = _branch;
            return commander.askForRepo (repos, 'Pick a repo to use as archive', 'name');
        })
        .then(function (_repo) {
            archiveRepo = _repo;
