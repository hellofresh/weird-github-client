module.exports = (function() {
    ...
    DeveloperStatsCommander.prototype.convertToReport = function(digested, columnProperty, rowProperty, valueProperty, sortProperty) {
        ...
        console.log ('---------- You are going to need these columns on your spreadsheet ----------');
    ...
    DeveloperStatsCommander.prototype.renderRepoStats = function (organization, digested) {
        ...
        result += '*Repository activity over the last 2 weeks.*\n';
    ...
    DeveloperStatsCommander.prototype.renderAuthorStats = function (organization, digested) {
        var result = '';
        var zeroes = [];

        result += '*Author activity over the last 2 weeks.*\n';
        result += 'Only contributions to each repository default repo are considered.\n';
        result += '\n';
        result += ':star: (>100), :sunny: (>85), :mostly_sunny: (>50), :partly_sunny: (>25), :partly_sunny_rain: (>10), :rain_cloud: (>3), :thunder_cloud_and_rain: (>0), :snowflake: (0)';
        result += '\n';
        ...
        for (var p in digested) {
            ...
            for (var q in digested [p]) {
                ...
                notes += '<https://github.com/' + organization + '/' + q + '/commits?author=' + p + '|' + q + '>';
            }
        ...
        for (var i = 0; i < zeroes.length; i++) {
            result += '<https://github.com/' + zeroes [i] + '|' + zeroes [i] + '>' + ((i == zeroes.length - 2) ? ' and ' : ((i == zeroes.length - 1) ? '.' : ', '));
        }
    ...
    DeveloperStatsCommander.prototype.snitch = function (organization, digested) {
        ...
        message += this.renderRepoStats (organization, this.convertDigestedToTree (digested, 'repo', 'author', 'commits'));
        message += '\n\n';
        message += this.renderAuthorStats (organization, this.convertDigestedToTree (digested, 'author', 'repo', 'commits'));
